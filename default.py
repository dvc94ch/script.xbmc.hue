...
class Hue:
    ...
    def update_settings(self):
        if (self.light is None or
                self.light.name != self.settings.light_name or
            ...
            if self.settings.light_type == 0:
                ...
                self.light = Light(self.settings.bridge_ip,
                                   self.settings.bridge_user,
                                   self.settings.light_name)

        if self.settings.backlight_enable:
            if (self.backlight is None or
                    self.backlight.name != self.settings.backlight_name or
                    self.backlight.group is not
                ...
                if self.settings.backlight_type == 0:
                    ...
                    self.backlight = Group(self.settings.bridge_ip,
                                           self.settings.bridge_user,
                                           self.settings.backlight_name)
                else:
                    self.backlight = Light(self.settings.bridge_ip,
                                           self.settings.bridge_user,
                                           self.settings.backlight_name)
        else:
            self.backlight = None
